Shader "Hidden/Theseus/VFX/ParticleEffect_CommonSF_Modular"
{
    Properties
    {
        // ══════════════════════════════════════════════════════════════
        ...
        // ══════════════════════════════════════════════════════════════
        // 【CustomData设置】 — 无开关（粒子特有）
        // ══════════════════════════════════════════════════════════════
        [HideInInspector][ModuleBegin(1)] _ModuleBegin_CustomData ("CustomData设置", Float) = 0
        [ModuleEnd][Toggle(_REQUIRE_CUSTOMDATA)]_RequireCustomData("开启CustomData(coord1.xy→DiffUV coord1.zw→MaskUV)", Float) = 0
        ...
        [HideInInspector][ModuleBegin(1)] _ModuleBegin_Main ("主贴图设置", Float) = 0
            ...
            [Vector4Split(FourFloats, Toggle, Toggle, Toggle, Toggle)]_BaseMapToggles("主贴图开关|开启预乘Alpha(禁动画中K开关)|去黑底(禁动画中K开关)|开启极坐标(禁动画中K开关)|切换为2U(禁动画中K开关)", Vector) = (0,0,0,0)
            [HDR]_BaseColor("整体叠色", Color) = (1,1,1,1)
            [HDR]_BaseFrontColor("前面叠色", Color) = (1,1,1,1)
            _FrontIntensity("颜色强度", Range(0, 10)) = 0
            [Vector4Split(FourFloats, Toggle, Slider, Slider, Hidden)]_DouYinEffectParams("抖音色效果|开启抖音色效果|OffsetX(-1, 1)|OffsetY(-1, 1)|_", Vector) = (0, 0.04, -0.04, 0)
        [ModuleEnd][Vector4Split(FourFloats, Float, Float, Slider, Slider)]_BaseUVParams("主贴图UV参数|U方向流速|V方向流速|缩放(0, 10)|旋转(0, 720)", Vector) = (0,0,1,0)
        ...
        [HideInInspector][ModuleBegin(1)] _ModuleBegin_Mask ("遮罩设置", Float) = 0
            ...
            [Vector4Split(FourFloats, Float, Float, Slider, Slider)]_MaskUVParams("遮罩UV参数|U方向流速|V方向流速|缩放(0, 10)|旋转(0, 720)", Vector) = (0,0,1,0)
            [Vector4Split(FourFloats, Toggle, Slider, Hidden, Hidden)]_MaskMapParams("遮罩参数|兼容纯Alpha图|遮罩强度(0, 1)|_|_", Vector) = (0,1,0,0)
            [Toggle]_EnableMaskMapPolarUV("开启极坐标", Float) = 0
        [ModuleEnd][Vector4Split(FourFloats, Toggle, Toggle, Toggle, Toggle)]_MaskMapToggles("遮罩通道开关|R通道不影响主贴图Alpha(禁动画中K开关)|R通道不影响混合贴图Alpha(禁动画中K开关)|G通道影响溶解(禁动画中K开关)|B通道影响扰动(禁动画中K开关)", Vector) = (0,0,0,0)
        ...
        [HideInInspector][ModuleBegin(_MIX_BASE_ON)] _ModuleBegin_MixBase ("混合贴图设置", Float) = 0
            ...
            [Vector4Split(FourFloats, Toggle, Toggle, Toggle, Slider)]_MixMapParams("混合参数|去黑|开启Noise影响|开启Ramp混色模式|混合贴图强度(0, 2)", Vector) = (0, 0, 0, 1)
        [ModuleEnd][Vector4Split(FourFloats, Float, Float, Slider, Slider)]_MixBaseMapUVParams("混合贴图UV参数|U方向流速|V方向流速|缩放(0, 10)|旋转(0, 720)", Vector) = (0,0,1,0)
		...
		[Vector4Split(TwoVector2, Float, Float)]_DissolveNoiseParam("溶解纹理参数|溶解纹理Tiling|溶解纹理UV流速", Vector) = (1,1,0,0)
		[Vector4Split(FourFloats, Slider, Slider, Slider, Slider)]_DissolveControlParams("溶解控制参数|溶解阈值(-2, 2)|溶解软硬(0, 1)|溶解边缘阈值(-2, 2)|溶解边缘软硬(0, 1)", Vector) = (0, 0.1, 0, 1)
        ...
        [ModuleEnd][Vector4Split(FourFloats, Float, Slider, Hidden, Hidden)]_FlowMapParams("FlowMap参数|流动速度|扰动强度(0, 1)|_|_", Vector) = (0.5, 1, 0, 0)
        ...
        [HideInInspector][ModuleBegin(_NOISE_ON)] _ModuleBegin_Noise ("扰动设置", Float) = 0
            ...
            [Vector4Split(FourFloats, Slider, Slider, Slider, Slider)]_NoiseStrength("扭曲强度|第一层扭曲强度U(-2, 2)|第一层扭曲强度V(-2, 2)|第二层扭曲强度U(-2, 2)|第二层扭曲强度V(-2, 2)", Vector) = (0.5, 0.5, 0.5, 0.5)
        ...
        [ModuleEnd][Vector4Split(FourFloats, Toggle, Slider, Slider, Enum)]_FresnelParams("Fresnel参数|反向Fresnel Alpha|范围(0, 2)|强度(0, 1)|叠加模式(Multiply,0,Add,1)", Vector) = (0, 0, 1, 0)
        ...
        [HideInInspector][ModuleBegin(_ENABLE_VERTEX_OFFSET)] _ModuleBegin_VertexOffset ("顶点偏移设置", Float) = 0
            ...
            [Vector4Split(FourFloats, Float, Float, Slider, Slider)]_VertexOffsetParams("顶点偏移参数|VertexScale|VertexPower|VertexScaleHeightU(0, 1)|VertexScaleHeightV(0, 1)", Vector) = (0, 1, 1, 0)
        ...
        [ModuleEnd][Vector4Split(FourFloats, Slider, Slider, Slider, Slider)]_GradientParams("渐变参数|UV权重(0, 1)|左侧渐变色权重(0, 1)|右侧渐变色权重(0, 2)|渐变色权重偏移(-1, 1)", Vector) = (0, 0, 1, 0)
        ...
        [HideInInspector][ModuleBegin(_COLOUR_ON)] _ModuleBegin_Colour ("调色设置", Float) = 0
            [Vector4Split(FourFloats, Slider, Slider, Slider, Hidden)]_ColorGradingParams("调色参数|色相(-0.5, 0.5)|饱和度(0, 2)|对比度(0, 2)|_", Vector) = (0, 1, 1, 0)
